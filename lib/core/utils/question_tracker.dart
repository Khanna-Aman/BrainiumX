...
class QuestionTracker {
  ...
  Future<void> markQuestionsUsed(GameId gameId, List<String> questionKeys) async {
    await initialize();
    final key = _getKey(gameId);
    final usedQuestions = await getUsedQuestions(gameId);
...
class QuestionKeyGenerator {
  ...
  static String patternMatrixKey(List<List<bool>> pattern) {
    final flatPattern = pattern.expand((row) => row).map((b) => b ? '1' : '0').join('');
    return 'pattern_$flatPattern';
  }
  
  /// Generate a key for word chain questions
  static String wordChainKey(String startWord, String endWord, List<String> chain) {
    return 'wordchain_${startWord}_${endWord}_${chain.join('_')}';
  }
  
  /// Generate a key for visual search questions
  static String visualSearchKey(List<String> symbols, Map<String, int> counts) {
    final sortedSymbols = symbols..sort();
    final countsStr = sortedSymbols.map((s) => '${s}:${counts[s]}').join('_');
    return 'visualsearch_$countsStr';
  }
