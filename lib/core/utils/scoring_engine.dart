class ScoringEngine {
  static double calculateComboScore(double baseScore, int streak) {
    return baseScore * (1 + (streak ~/ 5) * 0.25);
  }
  
  static double calculateTimeBonus(int timeBudgetMs, int elapsedMs, double weight) {
  ...
  static double calculateTrailConnectScore(int parTimeMs, int elapsedMs, int errors) {
  ...
  static double calculateVisualSearchScore(bool isTarget, bool correct) {
    if (isTarget && correct) return 40.0;
    if (!isTarget && !correct) return -50.0;
    return 0.0;
  }
}
